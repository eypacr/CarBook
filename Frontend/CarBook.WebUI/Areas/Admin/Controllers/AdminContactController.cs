...
public class AdminContactController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var responseMessage = await client.GetAsync("https://localhost:7204/api/Contacts/");
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var responseMessage = await client.DeleteAsync($"https://localhost:7204/api/Contacts?id={id}");
        if (responseMessage.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> CreateContact(CreateContactDto dto)
    {
        ...
        var responseMessage = await client.PostAsync("https://localhost:7204/api/Contacts", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
            return RedirectToAction("Index");
        }
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var responseMessage = await client.GetAsync($"https://localhost:7204/api/Contacts/{id}");
        var jsonData = await responseMessage.Content.ReadAsStringAsync();
    ...
    public async Task<IActionResult> UpdateContact(ContactUpdateDto contactUpdateDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
        var responseMessage = await client.PutAsync("https://localhost:7204/api/Contacts/", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
